...
from filepattern2 import FilePattern2
...
class InferenceTest(unittest.TestCase):
    ...
    def test_alphanumeric_channel_variable_width(self):
        ...
        pattern = FilePattern2.Pattern.infer_pattern(files=self.data['channel'])

        self.assertEqual(pattern,'img_r00{r:d}_c00{t:d}_{c:c+}.tif')
    
    def test_variable_naming(self):
        pattern = FilePattern2.Pattern.infer_pattern(files=self.data['channel'], variables='xyz')
        
        self.assertEqual(pattern,'img_r00{x:d}_c00{y:d}_{z:c+}.tif')

    def test_alphanumeric_both_variable_width(self):

        pattern = FilePattern2.Pattern.infer_pattern(files=self.data['both'])

        self.assertEqual(pattern, '{r:cccccc}_{t:d+}_{c:c+}.ome.tif')
    
    def test_invalid_input(self):
        with self.assertRaises(RuntimeError):
            pattern = FilePattern2.Pattern.infer_pattern(files=self.data['invalid'])

class ExternalInferenceTest(unittest.TestCase):

    path = 'FilePattern/tests/test_data/data'
    def test_fp_single_block(self):
        pattern = FilePattern2.Pattern.infer_pattern(path=self.path, block_size='1 GB')
        
        self.assertEqual(pattern, 'img_r00{r:d}_c00{t:d}_{c:c+}.tif')
        
    def test_fp_multiple_block(self):
        pattern = FilePattern2.Pattern.infer_pattern(path=self.path, block_size='1 MB')
        
        self.assertEqual(pattern, 'img_r00{r:d}_c00{t:d}_{c:c+}.tif')
        
    def test_sp_single_block(self):
        path = 'FilePattern/examples/test_data/sp_data.txt'
        pattern = FilePattern2.Pattern.infer_pattern(path=path, block_size='1 GB')
        
        self.assertEqual(pattern, 'img_r00{r:d}_c00{t:d}_{c:c+}.tif')
        
    def test_sp_multi_block(self):
        path = 'FilePattern/examples/test_data/sp_data.txt'
        pattern = FilePattern2.Pattern.infer_pattern(path=path, block_size='900 B')
    ...
    def test_vp_multi_block(self):
        path = 'FilePattern/tests/test_data/stitching-vector-61b3a82d6fa1f65bf8cc2448-img-global-positions-1.txt'
        pattern = FilePattern2.Pattern.infer_pattern(path=path, block_size='250 B')
