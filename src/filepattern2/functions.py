...
def infer_pattern(path: str='', files: list=[], variables: str='', block_size: str=''):
        ...
        if(path.endswith('.txt')):
            ...
            if(re.match(r'file\: .+?; corr\: .+?; position\: .+?; grid\: .+?;', line)):
                ...
                if(block_size == ''):
                    return backend.StringPattern.infer_pattern(path=path, files=files, variables=variables)
                else:
                    return backend.ExternalStringPattern.infer_pattern(path=path, files=files, variables=variables, block_size=block_size)
        else: # directory as input
            if(block_size == ''):
                return backend.InternalPattern.infer_pattern(path=path, files=files, variables=variables)
            else:
                return backend.ExternalFilePattern.infer_pattern(path=path, files=files, variables=variables, block_size=block_size)
        """